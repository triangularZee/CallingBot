...
function summaryPrompt({ title, transcriptText }) {
  return `
...
export async function summarizeWithGemini(transcriptText, { title = 'meeting' } = {}) {
  const ai = geminiClient();
  const response = await ai.models.generateContent({
    model: config.geminiModel,
    contents: summaryPrompt({ title, transcriptText })
  });
